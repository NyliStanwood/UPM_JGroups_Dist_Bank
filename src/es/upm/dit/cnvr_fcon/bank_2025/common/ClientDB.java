...
public class ClientDB implements Serializable {
	...
	//Logger is not serizalizable
	private static final long serialVersionUID = 1L;

	/**
	 * Data base of the bank
	 */
	...
	public void setClientDB ( java.util.HashMap <Integer, Client> db) {
		this.clientDB = db;
	}
	 ...
	 */
	public Client updateClient (int accNumber, int balance) {
		if (clientDB.containsKey(accNumber)) {
			Client client = clientDB.get(accNumber);
			client.setBalance(balance);
			clientDB.put(client.getAccountNumber(), client);
			return clientDB.get(accNumber);
		} else {
			return null;
		}	
	}
