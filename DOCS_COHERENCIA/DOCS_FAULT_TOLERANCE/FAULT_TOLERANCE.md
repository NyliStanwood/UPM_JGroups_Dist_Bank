...
## JGroups Configuration for Fault Tolerance

### XML-Based Protocol Stack

The system uses optimized JGroups XML configuration files (`resources/jgroups-udp.xml` and `resources/jgroups-tcp.xml`) to provide faster failure detection and recovery.

#### Key Protocols for Fault Tolerance

**1. FD_SOCK (Socket-Based Failure Detection)**
...
<FD_SOCK
  ...
  start_port="9000"
  port_range="100"
  suspect_msg_interval="2000"
/>
```

- **Purpose**: Primary failure detection mechanism
- **How it works**: Maintains direct TCP connections between nodes
- **Detection time**: ~2-3 seconds after socket timeout
- **Benefit**: Fast, reliable detection of crashed processes

**2. FD_ALL (Heartbeat-Based Failure Detection)**
...
- **Purpose**: Backup failure detection layer
- **How it works**: Periodic heartbeat messages (every 1 second)
- **Detection time**: 3 seconds after last heartbeat
- **Benefit**: Redundant detection for network partitions

**3. VERIFY_SUSPECT (False Positive Prevention)**

```xml
<VERIFY_SUSPECT timeout="1500" />
```

- **Purpose**: Confirm suspected failures before removal
- **How it works**: Double-checks suspected nodes
- **Benefit**: Prevents premature node removal
...
<GMS
  ...
  resume_task_timeout="7000"
  log_view_warnings="false"
/>
```

- **Purpose**: Manages cluster membership and view changes
- **Optimizations**:
  - `join_timeout="3000"` - Faster new node integration (down from 5000ms)
  - `leave_timeout="1000"` - Faster cleanup of failed nodes (down from 5000ms)
  - `max_bundling_time="300"` - Faster view updates (down from 500ms)
  - `resume_task_timeout="7000"` - Faster recovery (down from 15000ms)

**5. NAKACK2 (Reliable Multicast with Ordering)**
...
- **Purpose**: Ensures message ordering and coherence
- **Benefit**: Maintains consistency during view changes

**6. STATE (State Transfer Protocol)**

```xml
<STATE />
```

- **Purpose**: Automatic state transfer to new/rejoining nodes
- **Benefit**: New replicas immediately synchronized

#### Configuration Loading

NodeJG loads the XML configuration on startup:
...
try {
    ...
    LOGGER.warning("Failed to load XML config, using default: " + xmlError.getMessage());
...
| Metric            | Default Config  | Optimized XML Config | Improvement       |
| ----------------- | --------------- | -------------------- | ----------------- |
| Failure Detection | ~5-10 seconds   | ~3-4 seconds         | **40-50% faster** |
| View Update       | ~500ms          | ~300ms               | **40% faster**    |
| Join Time         | ~5 seconds      | ~3 seconds           | **40% faster**    |
| Recovery Start    | After detection | After detection      | Same              |
| Total Recovery    | ~7-15 seconds   | ~5-7 seconds         | **~30% faster**   |
...
### With Optimized JGroups XML Configuration

| Metric                  | Value            | Notes                                       |
| ----------------------- | ---------------- | ------------------------------------------- |
| Failure Detection Time  | ~3-4 seconds     | FD_SOCK + FD_ALL (optimized timeouts)       |
| View Update Time        | ~300ms           | GMS max_bundling_time optimization          |
| Quorum Check Time       | Immediate        | Checked in viewAccepted()                   |
| Recovery Decision       | Immediate        | If quorum lost and auto-recovery enabled    |
| Recovery Spawn Delay    | ~2 seconds       | Configurable delay between process spawns   |
| Process Launch Time     | ~1-2 seconds     | JVM startup + JGroups initialization        |
| State Transfer Time     | ~50-100ms        | Transfer complete ClientDB state            |
| **Total Recovery Time** | **~5-7 seconds** | **From failure to full quorum restoration** |

### Breakdown of Total Recovery Timeline

```
T+0s:  Node fails (crash)
T+3s:  FD_SOCK/FD_ALL detects failure
T+3.3s: View updated [Node1, Node3] (removed Node2)
T+3.3s: viewAccepted() called, quorum check fails
T+3.3s: scheduleProcessRecovery() spawns thread
T+5.3s: launchNewBankProcess() executes (after 2s delay)
T+6.3s: New process joins cluster
T+6.4s: State transfer completes
T+6.5s: View updated [Node1, Node3, Node4]
T+6.5s: Quorum restored ✓
```
